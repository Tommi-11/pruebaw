    ...
    <main class="flex-1 w-full">
        ...
        <section class="my-8">
            <div class="flex flex-col lg:flex-row items-center gap-8">
                <div class="lg:w-2/3 mb-3 lg:mb-0">
                    <div class="bg-gradient-to-r from-blue-700 to-blue-400 rounded-xl shadow-lg p-8 flex flex-col justify-center h-full">
                        <h1 class="text-4xl font-serif font-bold text-white mb-2">Bienvenidos a la Universidad Nacional Ejemplo</h1>
                        <p class="text-lg text-blue-100 mb-4 max-w-2xl">Nuestra universidad se compromete con la excelencia académica, la formación integral y la proyección social. Descubre nuestras carreras, servicios y actividades institucionales.</p>
                        <a href="#info" class="inline-block bg-white text-blue-800 font-semibold px-6 py-2 rounded shadow hover:bg-blue-50 transition">Conoce más</a>
                    </div>
                </div>
        ...
        <section class="bg-gray-50 py-8 border-t border-b">
            <div class="px-4 lg:px-0">
                <h2 class="font-serif text-3xl font-bold mb-6 text-blue-900">Últimas Noticias</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach($noticias as $noticia)
                        <div class="bg-white rounded-lg shadow-md flex flex-col overflow-hidden transition-transform transform hover:scale-105">
                            <img src="{{ $noticia->imagen }}" alt="{{ $noticia->titulo }}" class="h-32 w-full object-cover">
                            <div class="p-4 flex-1 flex flex-col">
                                <h3 class="font-semibold text-blue-800 text-lg mb-1">{{ $noticia->titulo }}</h3>
                                <p class="text-sm text-gray-600 flex-1">{{ $noticia->descripcion }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <section class="bg-white py-8 border-t">
            <div class="px-4 lg:px-0">
                <h2 class="font-serif text-3xl font-bold mb-6 text-blue-900">Calendario de Actividades</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($actividades as $mes => $eventos)
                        <div class="bg-blue-50 rounded-lg shadow flex flex-col">
                            <div class="p-5 flex-1 flex flex-col">
                                <h4 class="font-bold mb-2 text-blue-800">{{ $mes }}</h4>
                                <ul class="list-disc pl-6 text-sm text-gray-700 space-y-1">
        ...
        <!-- Nuestras Plataformas Web -->
        <section class="py-8">
            <h2 class="font-serif text-3xl font-bold mb-6 text-blue-900">Nuestras Plataformas Web</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($plataformas as $plataforma)
                    <div class="bg-white rounded-lg shadow p-5 text-center flex flex-col items-center transition-transform transform hover:scale-105">
                        <img src="{{ $plataforma->imagen }}" alt="{{ $plataforma->nombre }}" class="mb-3 h-12 w-auto">
                        <h5 class="font-bold text-blue-800 mb-1">{{ $plataforma->nombre }}</h5>
                        <a href="{{ $plataforma->link }}" class="text-blue-700 hover:underline text-sm font-medium">Ingresar</a>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        <!-- Enlaces de Interés -->
        <section class="py-8">
            <h2 class="font-serif text-3xl font-bold mb-6 text-blue-900">Enlaces de Interés</h2>
            <div class="flex flex-wrap gap-4 justify-center items-center">
                @foreach($enlaces as $enlace)
                    <a href="{{ $enlace->link }}" class="flex items-center transition-transform transform hover:scale-105">
                        <img src="{{ $enlace->imagen }}" alt="{{ $enlace->nombre }}" class="h-8 w-auto grayscale hover:grayscale-0 transition">
                    </a>
                @endforeach
            </div>
        </section>
