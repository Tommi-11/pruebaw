...
<div class="bg-white border-b border-gray-200 py-3">
    <div class="container mx-auto flex flex-col md:flex-row justify-between items-center px-4 gap-2">
        <div class="flex items-center gap-4">
            ...
            {{-- <div>
                <span class="block text-xs text-gray-400 tracking-widest uppercase font-medium">Universidad Nacional Ejemplo</span>
                <span class="block text-2xl font-serif font-bold text-blue-800 leading-tight">Rumbo a la Acreditación</span>
            </div> --}}
        </div>
        <div class="flex items-center gap-4 mt-2 md:mt-0">
            ...
            @guest
                <a href="{{ route('login') }}" class="text-blue-800 font-semibold hover:underline transition">Iniciar Sesión</a>
            @else
                <a href="{{ route('dashboard') }}" class="text-blue-800 font-semibold hover:underline transition">Panel</a>
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="text-blue-800 font-semibold hover:underline transition bg-transparent border-0 cursor-pointer">Cerrar Sesión</button>
                </form>
            @endguest
    ...
    <div class="max-w-screen-xl mx-auto px-4">
        ...
        <ul :class="{'block': open, 'hidden': !open}" class="lg:flex flex-col lg:flex-row lg:space-x-8 font-medium text-center w-full lg:w-auto bg-blue-900 lg:bg-transparent py-2 lg:py-0 rounded-b-lg lg:rounded-none shadow-lg lg:shadow-none z-40">
            ...
            <li class="relative" x-data="{ submenu: false }" @mouseenter="submenu = true" @mouseleave="submenu = false" @focusin="submenu = true" @focusout="submenu = false">
                <a href="{{ route('responsabilidad.social') }}" class="block py-2 px-4 text-white hover:bg-blue-800 rounded transition cursor-pointer">
                    Responsabilidad Social
                </a>
            ...
            <li class="relative" x-data="{ submenu: false }" @mouseenter="submenu = true" @mouseleave="submenu = false" @focusin="submenu = true" @focusout="submenu = false">
                <a href="{{ route('seguimiento.egresado') }}" class="block py-2 px-4 text-white hover:bg-blue-800 rounded transition cursor-pointer">
            ...
            <!-- PROYECCION SOCIAL -->
            <li class="relative" x-data="{ submenu: false }" @mouseenter="submenu = true" @mouseleave="submenu = false" @focusin="submenu = true" @focusout="submenu = false">
                <a href="{{ route('proyeccion.social') }}" class="block py-2 px-4 text-white hover:bg-blue-800 rounded transition cursor-pointer">
                    Proyección Social
                </a>
                <ul x-show="submenu" x-transition class="absolute left-1/2 -translate-x-1/2 mt-2 w-56 bg-white rounded shadow-xl z-30" @mouseenter="submenu = true" @mouseleave="submenu = false">
            ...
            <li class="relative" x-data="{ submenu: false }" @mouseenter="submenu = true" @mouseleave="submenu = false" @focusin="submenu = true" @focusout="submenu = false">
                <a href="{{ route('extension.universitaria') }}" class="block py-2 px-4 text-white hover:bg-blue-800 rounded transition cursor-pointer">
                    Extensión Universitaria
                </a>
